...
if(isMember==true){
	...
	request.setAttribute("ERROR_MSG", "넌 회원이 아님");
	request.getRequestDispatcher("Login.jsp").forward(request, response);
}


    
//방법2 : 회원테이블에서 레코드를 추출한 후 MemberDTO 객체에 저장 후 반환받기
...
if(memberDTO.getId() != null){
	...
	//저장된 값이 없다면..리퀘스트영역에 에러메세지를 저장하고 포워드
	request.setAttribute("ERROR_MSG", "넌 회원이 아님");
	request.getRequestDispatcher("Login.jsp").forward(request, response);
} 
...
if(memberInfo.get("id") != null){
	...
	//저장된 값이 없다면..리퀘스트영역에 에러메세지를 저장하고 포워드
	request.setAttribute("ERROR_MSG", "넌 회원이 아님");
	//response.sendRedirect("Login.jsp");//=>"넌회원이 아님은 출력되지 않았다."
	request.getRequestDispatcher("Login.jsp").forward(request, response);
}
%>
